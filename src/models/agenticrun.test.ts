...
describe('derivePhaseFromConditions', () => {
  ...
  it('returns NoActionRequired for Analyzed=True with reason NoActionRequired', () => {
    expect(derivePhaseFromConditions([cond('Analyzed', 'True', 'NoActionRequired')])).toBe('NoActionRequired');
  });

  it('Denied takes priority over NoActionRequired', () => {
    expect(derivePhaseFromConditions([cond('Denied', 'True'), cond('Analyzed', 'True', 'NoActionRequired')])).toBe('Denied');
  });
